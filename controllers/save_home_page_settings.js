/**
 * SaveHomePageSettings - Saves settings for the display of home page content in the Devblog theme
 *
...
SaveHomePageSettings.prototype.onPostParamsRetrieved = function(post, cb) {
    ...
    dao.query('devblog_theme_settings', {settings_type: 'home_page'}).then(function(homePageSettings) {
        if(homePageSettings.length > 0) {
            ...
            homePageSettings = pb.DocumentCreator.create('devblog_theme_settings', post, ['page_media']);
        ...
        dao.update(homePageSettings).then(function(result) {
            if(util.isError(result))  {
                self.formError(self.ls.get('ERROR_SAVING'), '/admin/plugins/settings/devblog-pencilblue/home_page', cb);
                return;
            }

            self.session.success = self.ls.get('HOME_PAGE_SETTINGS') + ' ' + self.ls.get('SAVED');
            delete self.session.fieldValues;
            cb(pb.RequestHandler.generateRedirect(pb.config.siteRoot + '/admin/plugins/settings/devblog-pencilblue/home_page'));
        });
    });
};
...
SaveHomePageSettings.getRoutes = function(cb) {
    var routes = [
        {
            method: 'post',
            path: '/actions/admin/plugins/settings/devblog-pencilblue/home_page',
            auth_required: true,
